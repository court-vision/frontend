 ...
 * one-line status, and Pause/Resume + Resync controls. Renders nothing when the
 * feature is switched off (no extension id) or before the room is enabled, so
 * the room looks exactly as it did without sync.
 */
export function DraftSyncChip({ sync }: { sync: EspnDraftSync }) {
  ...
  return (
    ...
    >
      ...
      <span className="max-w-[24ch] truncate text-foreground/80">{label}</span>
