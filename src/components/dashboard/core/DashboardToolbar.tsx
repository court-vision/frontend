...
interface DashboardToolbarProps {
  ...
  onOpenCatalog: () => void;
  /** Phone stack: the layout can't be edited, so Edit/Add/Reset are hidden. */
  readOnly?: boolean;
}
...
export function DashboardToolbar({
  ...
  const editing = isEditMode && !readOnly;

  return (
    <div
      className={cn(
        "flex items-center justify-between h-9 px-3 md:px-4 border-b border-border/40 bg-muted/10 shrink-0",
