...
function DefRatingDot({ rating }: { rating: number | null }) {
  if (rating === null) return <span className="inline-block w-1.5 h-1.5 rounded-full bg-muted-foreground/30 shrink-0" />;
  const color =
...
function GameRow({ game, isNext, rowRef }: GameRowProps) {
...
export function TeamSchedulePanel() {
  ...
  const { data: playerStats } = usePlayerStatsQuery(focusedPlayerId, "nba");
  const { data: rankings } = useRankingsQuery();

  const team = useMemo(() => {
    if (playerStats?.team) return playerStats.team;
    if (rankings && focusedPlayerId) {
      return rankings.find((r) => r.id === focusedPlayerId)?.team ?? null;
    }
    return null;
