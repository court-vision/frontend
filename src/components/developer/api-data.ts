...
export const categories: EndpointCategory[] = [
  {
    ...
    label: "Players",
    endpoints: [
      publicGet("/players/projections",
        "Paginated ESPN preseason per-game projections from one global snapshot. The date selector resolves on or before the requested date and the response reports the actual snapshot. Includes mapped rookies without game stats; rates are fractions (0–1) and missing values stay null.",
        [...snapshotParams,
        ...
        "/players/projections?season=2026-27&as_of=2026-09-05&name=Jokic&limit=25",
        { season: "2026-27", source: "espn", as_of_date: "2026-09-04", total: 1, limit: 25, offset: 0,
          players: [{ player_id: 203999, espn_id: 3112335, name: "Nikola Jokić", season: "2026-27", source: "espn", as_of_date: "2026-09-04", projected_gp: 72,
...
r = requests.get("${API_BASE}/players/2544/percentiles",
  ...
  },
  {
    ...
    endpoints: [
      ...
      publicGet("/rankings/espn/movement",
        "Compare the union of two independently resolved ESPN market snapshots. Positive rank/ADP changes mean improvement; positive auction changes mean increased value. Entrants, exits, and missing measurements stay null. If two distinct snapshots cannot resolve, the collection is empty with an explanation.",
        [
          { name: "from_as_of", type: "string", required: true, description: "Resolve the before snapshot on or before YYYY-MM-DD" },
          { name: "to_as_of", type: "string", required: true, description: "Resolve the after snapshot on or before YYYY-MM-DD; must be later than from_as_of" },
          { name: "season", type: "string", required: false, description: "Consecutive NBA season key, e.g. 2026-27; defaults to the active season" },
          { name: "name", type: "string", required: false, description: "Case- and accent-insensitive name search" },
          { name: "metric", type: "string", required: false, description: "rank (default), adp, auction_value, or auction_value_avg" },
          { name: "direction", type: "string", required: false, description: "up, down, or both (default)" },
          { name: "limit", type: "integer", required: false, description: "Page size (1–100, default 50)" },
          { name: "offset", type: "integer", required: false, description: "Pagination offset (default 0)" },
        ],
        "/rankings/espn/movement?season=2026-27&from_as_of=2026-09-01&to_as_of=2026-09-08&metric=rank&direction=up",
        { season: "2026-27", source: "espn", from_as_of_date: "2026-09-01", to_as_of_date: "2026-09-07", metric: "rank", direction: "up", total: 1, limit: 50, offset: 0,
          players: [{ player_id: 203999, espn_id: 3112335, name: "Nikola Jokić",
            before: { overall_rank: 8, adp: 7.4, auction_value: 54, auction_value_avg: 52.1 },
            after: { overall_rank: 5, adp: 5.8, auction_value: 58, auction_value_avg: 55.2 },
            changes: { overall_rank: 3, adp: 1.6, auction_value: 4, auction_value_avg: 3.1 } }] }),
      {
        method: "GET",
        path: "/rankings/",
